...
/*   Updated: 2024/04/04 19:18:34 by ykerdel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Users *Server::createUser(int socketDescriptor) {
  ...
  if (this->getUserByUn(userName) != NULL) {
    std::cout << "@" << (this->getUserByUn(userName))->getNickName()
              << " rejoined the server" << std::endl;
  ...
  Users *newUser = new Users(nickName, userName, defaultModes,
                             defaultPermissionLevel, socketDescriptor);
...
void Server::start() {
  ...
  while (true) {
    ...
    if (FD_ISSET(this->serverSocket, &readfds)) {
      ...
      int cs = accept(this->serverSocket, (struct sockaddr *)&clientAddress,
                      &clientAddrSize);
      if (cs == -1) {
      ...
      if (!user) {
        std::cerr << "Error: failed to create user for the connected client."
                  << std::endl;
        close(cs);
        continue;
      }
      if (user->getSocketDescriptor() != cs || !user->getStatus()) {
        ...
        this->all_users.insert(
            std::pair<std::string, Users *>(user->getUserName(), user));
      FD_SET(cs, &readfds);
      if (cs >= max_sd)
        max_sd = cs;
      user = NULL;
    ...
    for (std::map<std::string, Users *>::iterator it = this->all_users.begin();
      ...
      if (FD_ISSET(clientSocket, &readfds) && it->second->getStatus()) {
        ...
        if (bytesReceived == 0) {
          ...
          std::cerr << "Connection closed by @" << it->second->getNickName()
                    << std::endl;
          continue;
        } else if (bytesReceived < 0) {
          std::cerr << "Error receiving msg from @" << it->second->getNickName()
                    << std::endl;
          continue;
        }
        std::string msg(buffer, bytesReceived - 1);
        
		Message input = parsing(msg);
        // parse msg, type std::string
        std::cout << "@" << it->second->getNickName() << ": " << msg
                  << std::endl;
      }
    }
  }
